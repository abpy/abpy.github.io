...
###### Manual
The values must be carefully chosen to find a best fit that works for a variety of images. The specific values used will depend on all the details of your scanning process, but with everything kept consistent the values can be used for all frames scanned from a particular film stock. Depending on personal preference or lighting conditions, some frames can be adjusted differently from the best fit density balance.
