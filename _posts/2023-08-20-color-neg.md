...
##### Density Balance
A digital camera does not see the cyan, magenta, and yellow dyes of the film as equal in density. This causes a color shift in denser parts of the scan. To compensate, we need to scale the density of each channel separately. By adjusting the density of each channel, we change the slope or contrast of the film's characteristic curve. This allows us to equal out the densities so that gray stays neutral throughout the exposure range of the film. Density balance and white balance work together to define neutral gray. Changes to density balance must be offset by changing the white balance.
...
If adjusting manually, leave red at 1.0 and reduce the green and blue channels. The blue channel adjusts blue/yellow contrast, and the green adjusts green/magenta. If highlights are too blue and shadows too yellow, the blue contrast is too high and needs to be set lower. The process is similar for the other channel. Repeat on both channels while also adjusting white balance until shadows, midtones, and highlights are all neutral.
...
Because density uses a logarithm, it changes our math from a power function: `(x * w)^d`, to a linear function: `(x + w) * d`, which I think is easier to conceptualize than a curved one.
...
2. add solid color layer on add (default black)
3. add solid color layer on multiply (default white)
4. add density to luminance lut
5. adjust color layers
...
1. add density lut adjustment layer
2. use eyedropper tool to sample density of middle gray area of the photo
...
### Automation
The creation of the layers can be automated with an action, and, if using a color checker density balance can be calculated and automated by a [script](https://github.com/abpy/abpy.github.io).
...
I will soon add a python script for building a negative to positive 3d lut.
