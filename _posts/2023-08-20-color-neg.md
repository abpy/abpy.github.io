...
> * backlight: evenly lit, smooth, block-out light contamination
>   * can use: broad spectrum white, flash, rgb led, blue filter (like 80b or similar)
...
The split method is easier and a good place to start before finding what looks best for your camera and film. Also, because Adobe Standard profiles often include gamut compression, it's best to set the raw white balance to the film base.
...
To adjust manually, we'll reduce the density/contrast of the green and blue channels to match the red channel. The blue channel adjusts blue/yellow contrast, and the green channel adjusts green/magenta. If highlights are too blue and shadows too yellow, the blue contrast is too high and needs to be set lower. The process is similar for the other channel. Repeat on both channels while also adjusting white balance until shadows, midtones, and highlights are all neutral.
...
(Due to a bug in recent versions of Photoshop, the exposure adjustment layer does not work correctly in 16 bit mode unless the RGB Working Space in Color Settings matches the document color profile. 32 bit mode can also be used instead.)
...
This was written in 2023 without the use of AI
Copyright 2023 - 2026 Aaron Buchler
